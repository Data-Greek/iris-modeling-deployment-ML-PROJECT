...
def mayou():
    ...
    if selection == "Prédiction en temps réel":
        ...
        sepal_length = st.slider('Indiquez la longueur des sépales en mm :', min_value=0.1, max_value=9.8, value=2.5)
        sepal_width	= st.slider('Indiquez la largeur des sépales en mm :', min_value=0.1, max_value=9.9, value=1.5)
        petal_length = st.slider('Indiquez la longueur des pétales en mm :', min_value=0.1, max_value=9.9, value=0.5)	
        petal_width = st.slider('Indiquez la largeur des pétales en mm :', min_value=0.1, max_value=9.9, value=3.5)
        
        ## Prédiction finale 
        resultat = ""
        if st.button("Prédiction"):
            resultat = predict(sepal_length, sepal_width, petal_length, petal_width)
            st.success("Ces résultats sont caractéristique d'une fleur de la famille des **{}**".format(resultat))
